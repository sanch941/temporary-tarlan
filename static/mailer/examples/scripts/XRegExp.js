...
(function(undefined) {
    ...
    XRegExp = function(pattern, flags) {
        const output = [];
        let currScope = XRegExp.OUTSIDE_CLASS;
        let pos = 0;
        let context;
        let tokenResult;
        let match;
    ...
    const replacementToken = /\$(?:(\d\d?|[$&`'])|{([$\w]+)})/g;
    ...
    const compliantExecNpcg = nativ.exec.call(/()??/, '')[1] === undefined; // check `exec` handling of nonparticipating capturing groups
    const compliantLastIndexIncrement = (function() {
        const x = /^/g;
        nativ.test.call(x, '');
        return !x.lastIndex;
    })();
    const hasNativeY = RegExp.prototype.sticky !== undefined;
    var nativeTokens = {};
    ...
    XRegExp.execAt = function(str, regex, pos, anchored) {
        const r2 = clone(regex, 'g' + (anchored && hasNativeY ? 'y' : ''));
        let match;
    ...
    XRegExp.matchChain = function(str, chain) {
        return (function recurseChain(values, level) {
            const item = chain[level].regex
                ? chain[level]
                : { regex: chain[level] };
            const regex = clone(item.regex, 'g');
            const matches = [];
            let i;
    ...
    String.prototype.replace = function(search, replacement) {
        ...
        if (
            ...
            result = nativ.replace.call(str, search, function() {
                const args = arguments; // Keep this function's `arguments` available through closure
    ...
    String.prototype.split = function(s /* separator */, limit) {
        ...
        const str = this + ''; // Type conversion
        const output = [];
        let lastLastIndex = 0;
        let match;
        let lastLength;
    ...
    XRegExp.addToken(/\\k<([\w$]+)>/, function(match) {
        const index = indexOf(this.captureNames, match[1]);
